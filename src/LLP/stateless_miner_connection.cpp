...
namespace LLP
{
    ...
    bool StatelessMinerConnection::sign_block(uint64_t nNonce, const uint512_t& hashMerkleRoot)
    {
        ...
        if(pBlock)
        {
            ...
            /* For prime channel, calculate offsets */
            if(pBlock->nChannel == 1)
            {
                debug::log(0, "   Calculating prime offsets...");
                TAO::Ledger::GetOffsets(pBlock->GetPrime(), pBlock->vOffsets);
                debug::log(0, ANSI_COLOR_BRIGHT_GREEN, "   ✓ Offsets calculated: ", pBlock->vOffsets.size(), " offsets", ANSI_COLOR_RESET);
            }
