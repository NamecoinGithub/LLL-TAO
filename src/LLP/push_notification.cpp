...
namespace LLP
{
    ...
    uint16_t PushNotificationBuilder::GetNotificationOpcode(uint32_t nChannel, ProtocolLane lane)
    {
        if (lane == ProtocolLane::LEGACY)
        {
            ...
            if (nChannel == 1)
                ...
                throw std::invalid_argument("Invalid channel for push notification: " + std::to_string(nChannel));
        }
        else // ProtocolLane::STATELESS
        {
            // 16-bit mirror-mapped opcodes for stateless lane (port 9323+)
            if (nChannel == 1)
    ...
    /* BuildChannelNotification - Template specialization for 8-bit (legacy lane) */
    template<>
    ...
    {
        // Validate lane (must be LEGACY for 8-bit Packet)
        if (lane != ProtocolLane::LEGACY)
            throw std::invalid_argument("Packet template requires LEGACY lane (got STATELESS)");
        ...
        debug::log(2, "   Opcode: 0x", std::hex, static_cast<uint32_t>(nOpcode), std::dec, " (", 
            nChannel == 1 ? "PRIME_BLOCK_AVAILABLE" : "HASH_BLOCK_AVAILABLE", ")");
        debug::log(2, "   Lane: 8-bit (LEGACY, port 8323)");
        debug::log(2, "   Payload: 12 bytes");
        debug::log(2, "   Heights: unified=", stateBest.nHeight, 
                   ", channel=", stateChannel.nChannelHeight, 
                   ", diff=0x", std::hex, nDifficulty, std::dec);

        return packet;
    }

    /* BuildChannelNotification - Template specialization for 16-bit (stateless lane) */
    template<>
    ...
    {
        // Validate lane (must be STATELESS for 16-bit StatelessPacket)
        if (lane != ProtocolLane::STATELESS)
            throw std::invalid_argument("StatelessPacket template requires STATELESS lane (got LEGACY)");
        ...
        debug::log(2, "   Lane: 16-bit (STATELESS, port 9323+)");
        debug::log(2, "   Payload: 12 bytes");
        debug::log(2, "   Heights: unified=", stateBest.nHeight, 
                   ", channel=", stateChannel.nChannelHeight, 
                   ", diff=0x", std::hex, nDifficulty, std::dec);
    ...
    /* BuildStatelessTemplate - Stateless-only feature (228-byte template push) */
    ...
    )
    {
        ...
        if (vBlockData.size() != 216)
        {
            ...
            throw std::runtime_error("Invalid block template size: " + std::to_string(vBlockData.size()));
        }
